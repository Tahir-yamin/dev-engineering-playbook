...
Usage:
    ...
    # Website application (Interactive Subagent Mode)
    python apply_job.py --company "AECOM" --role "Project Controls Manager" --website --url "https://aecom.jobs/xyz"
"""
...
class UnifiedJobApplication:
    ...
    def apply(self, company: str, role: str, email: str = None, website: bool = False, 
        ...
        if email:
            ...
            print(f"\nStep 3: Launching Interactive Autofill...")
            self._interactive_autofill(company, role, cv_path, cl_path, self.apply_url)
        else:
    ...
    def _interactive_autofill(self, company: str, role: str, cv_path: str, cl_path: str, apply_url: str):
        """Prepare data for the agent to perform interactive autofill."""
        print(f"\nPREPARING AUTOFILL FOR: {apply_url}")
        
        # Load profile data for the prompt
        with open(PROFILE_PATH, 'r', encoding='utf-8') as f:
            profile = json.load(f)
            
        # Create a summary of instructions for the agent
        prompt = {
            "action": "AUTOFILL_JOB_APPLICATION",
            "url": apply_url,
            "company": company,
            "role": role,
            "files": {
            ...
            "profile_summary": {
                ...
                "ats_disclosures": profile["personal"].get("ats_disclosures", {}),
                "experience_summary": [f"{e['title']} at {e['company']}" for e in profile.get("experience", [])[:3]],
                "education": profile.get("education", []),
                "skills": profile.get("technical_skills", [])
            }
        }
        
        # Save this prompt to a temporary file so I (the agent) can read it
        prompt_path = os.path.join(GENERATED_DIR, f"autofill_prompt_{company.replace(' ', '_')}.json")
        with open(prompt_path, 'w', encoding='utf-8') as f:
            json.dump(prompt, f, indent=4)
            
        print(f"Autofill instructions saved to: {prompt_path}")
        print(f"AGENT ACTION REQUIRED: Please read this file and use the browser tool to complete the application.")
        ...
        latest_path = os.path.join(GENERATED_DIR, "latest_app.json")
        with open(latest_path, 'w', encoding='utf-8') as f:
            json.dump(latest_state, f, indent=4)
            
        # Also generate the HTML fallback (silent)
        self._generate_website_data(company, role, cv_path, cl_path, apply_url, silent=True)

    def _generate_website_data(self, company: str, role: str, cv_path: str, cl_path: str, apply_url: str = None, silent: bool = False):
