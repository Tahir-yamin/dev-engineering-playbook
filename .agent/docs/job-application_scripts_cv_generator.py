...
class CVGenerator:
    ...
    def generate_cv(self, job_title: str = None, company: str = None, 
        ...
        # Combine profile skills with JD-extracted keywords for ATS optimization
        all_skills = set(self.profile["technical_skills"])
        if job_description:
            jd_keywords = self.extract_keywords(job_description)
            # Normalize and add (capitalizing for consistency)
            for kw in jd_keywords:
                all_skills.add(kw.title())
        
        skills = doc.add_paragraph()
        skills.add_run(" | ".join(sorted(list(all_skills)))).font.size = Pt(10)
