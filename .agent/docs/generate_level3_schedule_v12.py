...
tasks = [
    ...
    # FIXED: SW -> FS (Finish-to-Start)
    {"WBS": "1.2.2.1.8", "Task Name": "Bitumen & Backfilling", "Duration": "3 days", "Predecessors": "1.2.2.1.7FS", "Outline Level": 5}, 
    ...
    # 2.2.4 Finishing
    # FIXED: SW -> FS
    {"WBS": "1.2.2.4", "Task Name": "Finishing Works", "Duration": "30 days", "Predecessors": "1.2.2.2.5FS", "Outline Level": 4}, 
    {"WBS": "1.2.2.4.1", "Task Name": "Internal Plaster & [QC] Curing (5 Days)", "Duration": "8 days", "Predecessors": "1.2.2.2.5FS", "Outline Level": 5},
    ...
    {"WBS": "1.2.3.2", "Task Name": "[QC] Foundation Curing (7 Days)", "Duration": "7 days", "Predecessors": "1.2.3.1", "Outline Level": 4},
    ...
    {"WBS": "1.2.5.1", "Task Name": "Underground Tanks & Manholes", "Duration": "15 days", "Predecessors": "1.2.1.2", "Outline Level": 4},
...
def resolve_preds(pred_str):
    ...
    for p in preds:
        p = p.strip()
        if not p: continue
        # Handle typo "SW" (Start after Wait) which meant FS (Finish Start)
        p = p.replace("SW", "FS") 
...
output_file = "d:/my-dev-knowledge-base/Project_Schedule_MSP_Export_v12.xlsx"
df.to_excel(output_file, index=False)
print(f"Schedule V12 (Fixed Predecessor Notation) exported to {output_file}")
